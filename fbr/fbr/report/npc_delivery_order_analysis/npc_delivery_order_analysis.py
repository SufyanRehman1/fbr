...
def get_data(conditions, filters):
    data = frappe.db.sql(
        ...
        SELECT
            ...
            soi.prevdoc_docname as prevdoc_docname,
            -- ADD THIS LINE to get quotation quantity
            (SELECT qi.qty FROM `tabQuotation Item` qi WHERE qi.parent = soi.prevdoc_docname AND qi.item_code = soi.item_code LIMIT 1) as quotation_qty
        FROM
            `tabSales Order` so,
